...
describe('GraniteClient', () => {
  it('posts a deterministic private-archive grounded request with the complex answer ceiling', async () => {
    ...
    await expect(client.generate('Compare Story and Vault evidence.', 'Source 1\nShe was born in Jinja.')).resolves.toBe('Grandmother was born in Jinja.')
    ...
    expect(user).toContain('Private source context:')
    expect(user).toContain('Compare Story and Vault evidence.')
    expect(user).toContain('Source 1\nShe was born in Jinja.')
    expect(user).not.toContain('Vault source context:')
  })
