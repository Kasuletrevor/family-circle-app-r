...
describe('LegacyCircleAuthAdapter management writes', () => {
  ...
  it('marks notifications read through the authenticated compatibility endpoint', async () => {
    ...
    await expect(adapter.markNotificationsRead('88')).resolves.toEqual({ success: true })

    expect(calls).toEqual([{
      path: '/api/me/88/notifications/read',
      method: 'POST',
      body: {},
    }])
  })
