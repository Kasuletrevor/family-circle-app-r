...
describe('Windows packaging workflow', () => {
  ...
  it('creates the immutable tag and GitHub Release only after verified server deployment', () => {
    ...
    expect(finalizeBlock).toContain('GH_REPO: ${{ github.repository }}')
    ...
    expect(finalizeBlock).not.toContain('--clobber')
    expect(finalizeBlock).toContain("gh release view \"$RELEASE_TAG\" --json assets")
    expect(finalizeBlock).toContain('gh release download "$RELEASE_TAG" --pattern "$expected_installer"')
    expect(finalizeBlock).toContain('Existing GitHub Release installer checksum mismatch for $RELEASE_TAG')
    expect(finalizeBlock).toContain('Existing GitHub Release checksum asset mismatch for $RELEASE_TAG')
