...
describe('demo server publish contract', () => {
  it('publishes verified main builds to the existing demo release root', () => {
    ...
    expect(workflow).toContain('release_eligible="false"')
    expect(workflow).toContain('feat|fix|perf|refactor|chore|ci|docs)')
    expect(workflow).toContain('echo "release_eligible=$release_eligible" >> "$GITHUB_OUTPUT"')
    expect(workflow).toContain("steps.meta.outputs.release_eligible == 'true'")
    expect(workflow).toContain('stage="release-skipped"')
    expect(workflow).toContain('RELEASE_ELIGIBLE: ${{ needs.deploy-demo.outputs.release_eligible }}')
